...
async def test_close_user_channels_multiple_connections(user):
    ...
    await sync_to_async(close_user_channels)(user.pk)
    assert not await c_u11.queue_empty()
    assert not await c_u12.queue_empty()
    assert not await c_u13.queue_empty()
    assert await c_u21.queue_empty()
    assert await c_u22.queue_empty()
    assert await c_u31.queue_empty()
    assert await c_u32.queue_empty()
    assert await c_n1.queue_empty()
    assert c_n2.output_queue.empty()
    assert c_n3.output_queue.empty()
    ...
    await sync_to_async(close_user_channels)('')
    assert await c_u11.queue_empty()
    assert await c_u12.queue_empty()
    assert await c_u13.queue_empty()
    assert await c_u21.queue_empty()
    assert await c_u22.queue_empty()
    assert await c_u31.queue_empty()
    assert await c_u32.queue_empty()
    assert not await c_n1.queue_empty()
    assert not c_n2.output_queue.empty()
    assert not c_n3.output_queue.empty()
